...
export default function MontresAdmin() {
  ...
  async function handleDelete(id: string, name: string) {
    ...
    setDeleting(id)
    await fetch(`/api/admin/montres/${id}`, { method: 'DELETE' })
    await load()
    setDeleting(null)
  }
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gold-900/40">
                <th className="text-left text-neutral-400 font-medium px-5 py-4">Montre</th>
                <th className="text-left text-neutral-400 font-medium px-5 py-4 hidden sm:table-cell">Marque</th>
                <th className="text-left text-neutral-400 font-medium px-5 py-4 hidden md:table-cell">Genre</th>
              ...
              {filtered.map(montre => (
                <tr key={montre.id} className="hover:bg-neutral-800/50 transition-colors">
                  ...
                  <td className="px-5 py-4 hidden sm:table-cell">
                    <span className="bg-blue-900/30 text-blue-400 text-xs px-2.5 py-1 rounded-full border border-blue-800/40">
                      {montre.brand}
                    </span>
                  </td>
                  ...
                  <td className="px-5 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button
                        onClick={() => handleDelete(montre.id, montre.name)}
